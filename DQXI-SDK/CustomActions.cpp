...
void Init_CustomActions()
{
  ...
  MH_GameHook(AJackFieldPlayerController__InitActionMappings);

  if (!Options.CustomActions)
    return;

  MH_GameHook(AActor__InitActionMappingsUI);
  MH_GameHook(AJackTripleManager__SetupPlayerInputComponent);
}
